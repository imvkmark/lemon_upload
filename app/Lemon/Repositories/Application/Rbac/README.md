...
```php
Imvkmark\L5Rbac\L5RbacServiceProvider::class,
```
in the `providers` array and
```php
'Rbac'   => \Imvkmark\L5Rbac\Facades\L5Rbac::class,
```
...
These values will be used by rbac to refer to the correct users table and model.

You can also publish the configuration for this package to further customize table names and model namespaces.
Just use `php artisan vendor:publish` and a `l5-rbac.php` file will be created in your app/config directory.

### User relation to roles

Now generate the L5Rbac migration:

```bash
php artisan l5-rbac:migration
```

It will generate the `<timestamp>_l5-rbac_setup_tables.php` migration.
...
use Imvkmark\L5Rbac\Models\L5RbacRole;

class L5Role extends L5RbacRole
{
}
```
...
use Imvkmark\L5Rbac\Models\L5RbacPermission;

class L5Permission extends L5RbacPermission
{
}
```
...
use Imvkmark\L5Rbac\Traits\L5RbacUserTrait;

class User extends Eloquent
{
    use L5RbacUserTrait;  // add this trait to your user model

    // ...
}
```

This will enable the relation with `Role` and add the following methods `roles()`, `hasRole($name)`, `capble($permission)`, and `ability($roles, $permissions, $options)` within your `User` model.
...
$user->capable(['edit-user', 'create-post']);       // true
$user->capable(['edit-user', 'create-post'], true); // false, user does not have edit-user permission
```

You can have as many `Role`s as you want for each `User` and vice versa.

The `Rbac` class has shortcuts to both `can()` and `hasRole()` for the currently logged in user:
...
```php
Rbac::ability('admin,owner', 'create-post,edit-user');
...
@permission('manage-admins')
    ...
    \Rbac::capable('manage-admins'). The @can directive is already taken by core
    laravel authorization package, hence the @permission directive instead.</p>
@endpermission

@ability('admin,owner', 'create-post,edit-user')
    <p>This is visible to users with the given abilities. Gets translated to
    \Rbac::ability('admin,owner', 'create-post,edit-user')</p>
@endability
```
...
As you can see `Rbac::hasRole()` and `Rbac::capable()` checks if the user is logged in, and then if he or she has the role or permission.
