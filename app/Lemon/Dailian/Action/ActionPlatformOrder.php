...
use App\Models\PlatformOrderStar;
...
class ActionPlatformOrder extends ActionBasic {
	 ...
	 * @return bool
	 */
	public function create($input, $publish_id) {

		$input['publish_id']           = $publish_id;
		$input['publish_account_name'] = PamAccount::getAccountNameByAccountId($publish_id);

		\DB::transaction(function () use ($input, $publish_id) {
			// for pay
			...
			// progress log
			$log = '[订单创建] 订单ID: ' . $order->order_id .
				', 发单账号: ' . $input['publish_account_name'];
			PlatformLog::record($order, PlatformLog::LOG_BY_PLATFORM, PlatformLog::LOG_TYPE_CREATE, $log);
	...
	public function edit($order_id, $input, $editor_id = 0) {
		$order = PlatformOrder::info($order_id);
		...
		\DB::transaction(function () use ($input, $pam, $order, $diffPrice, $editor_id) {
			...
			PlatformOrder::where('order_id', $order->order_id)->update($input);

			// progress log
			$log = '[订单编辑] 订单号: ' . $order->order_no;
			PlatformLog::record($order->order_id, $editor_id, PlatformLog::LOG_TYPE_PUBLISH, $log);
	...
	public function handle($order_id, $account_id, $editor_id = 0) {
		...
		\DB::transaction(function () use ($order, $pam, $editor_id) {
			...
			// 游戏日志
			PlatformLog::record($order->order_id, $doer_id, PlatformLog::LOG_TYPE_ASSIGN, $msg);

			// 订单状态, 接单时间
			$ended_at  = Carbon::now()->addHours($order->order_hours)->toDateTimeString();
			$orderInfo = [
				'order_status'    => PlatformOrder::ORDER_STATUS_ING,
			...
			PlatformOrder::where('order_id', $order->order_id)
				->update($orderInfo);
	...
	public function addTime($order_id, $hour, $editor_id) {
		\DB::transaction(function () use ($hour, $order_id, $editor_id) {
			...
			// log
			PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_ADD_TIME, $log);
	...
	public function addMoney($order_id, $amount, $editor_id) {
		\DB::transaction(function () use ($order_id, $amount, $editor_id) {
			...
			// log
			PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_ADD_MONEY, '[补款]' . $log);
	...
	public function exception($order_id, $message, $exception_type, $account_id) {
		...
		\DB::transaction(function () use ($order_id, $message, $exception_type, $account_id) {
			// log
			$exceptionDesc = PlatformOrder::exceptionTypeDesc($exception_type);
			$log           = "[提交异常] 异常类型: {$exceptionDesc}, 说明:" . $message;
			PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_EXCEPTION, $log);

			// update order
			PlatformOrder::where('order_id', $order_id)->update([
				'order_status'   => PlatformOrder::ORDER_STATUS_EXCEPTION,
	...
	public function cancelException($order_id, $editor_id) {
		...
		PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_CANCEL_EXCEPTION, '[取消异常]');
	...
	public function talk($order_id, $message, $editor_id) {
		...
		if ($data) {
			PlatformOrder::where('order_id', $order_id)->update($data);
		}

		PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_PROGRESS, $message);
		return true;
	}
	...
	public function submitOver($order_id, $message, $pic_key, $editor_id) {
		\DB::transaction(function () use ($pic_key, $order_id, $message, $editor_id) {
			...
			// over log
			PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_SUBMIT_OVER, '[完成待审] 提交订单至待审核状态, 请发单者尽快登陆游戏验收并修改密码! 72 小时后系统将自动结算');

			// 订单状态
			PlatformOrder::where('order_id', $order_id)->update([
				'overed_at'    => Carbon::now(),
				'order_status' => PlatformOrder::ORDER_STATUS_EXAMINE,
			]);
	...
	public function over($order_id, $account_id) {
		\DB::transaction(function () use ($order_id, $account_id) {
			...
			PlatformOrder::where('order_id', $order_id)->update([
				'ended_at'     => Carbon::now(),
				'order_status' => PlatformOrder::ORDER_STATUS_OVER,
			]);
			...
			// log
			PlatformLog::record($order->order_id, $account_id, PlatformLog::LOG_TYPE_OVER, '[订单完成]审核并支付代练金!');
	...
	public function lock($order_id, $reason, $account_id) {
		...
		$lockLog = '[锁定订单] ' . $reason;
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_LOCK, $lockLog);
	...
	public function unlock($order_id, $reason, $account_id) {
		...
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_UNLOCK, $unlockLog);
	...
	public function changeGameAccount($order_id, $game_account, $game_pwd, $account_id) {
		...
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_EXCEPTION, '账号密码已经修改!');
	}
	...
	public function pubCancel($order_id, $pub_pay, $sd_pay, $account_id, $reason = '') {
		$data = [
			'order_status'      => PlatformOrder::ORDER_STATUS_CANCEL,
			'cancel_type'       => PlatformOrder::CANCEL_TYPE_PUB_DEAL,
			'cancel_status'     => PlatformOrder::CANCEL_STATUS_ING,
			'msg_pub_cancel'    => 'Y',
		...
		$log = rtrim($log, ', ');
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_PUB_CANCEL, $log);
	}
	...
	public function pubCancelAgree($order_id) {
		...
		\DB::transaction(function () use ($order, $toSoldier, $toPublisher, $returnToPublisher, $returnToSoldier) {
			...
			$data = [
				'order_status'     => PlatformOrder::ORDER_STATUS_CANCEL,
				'cancel_status'    => PlatformOrder::CANCEL_STATUS_DONE,
				'cancel_passed_at' => Carbon::now(),
			];
			PlatformOrder::where('order_id', $order->order_id)->update($data);
	...
	public function pubCancelReject($order_id, $account_id) {
		PlatformOrder::where('order_id', $order_id)->update([
			'pub_pay'            => 0,
			'sd_pay'             => 0,
			'cancel_status'      => PlatformOrder::CANCEL_STATUS_REJECT,
			'cancel_rejected_at' => Carbon::now(),
		]);

		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_CANCEL, '[拒绝撤销方案] 接单者不同意您的撤销方案!');
	...
	public function sdCancel($order_id, $pub_pay, $sd_pay, $account_id) {
		$data = [
			'order_status'      => PlatformOrder::ORDER_STATUS_CANCEL,
			'cancel_type'       => PlatformOrder::CANCEL_TYPE_SD_DEAL,
			'cancel_status'     => PlatformOrder::CANCEL_STATUS_ING,
			'msg_sd_cancel'     => 'Y',
			'cancel_applied_at' => Carbon::now(),
			'pub_pay'           => $pub_pay,
		...
		$log = rtrim($log, ', ');
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_CANCEL, $log);
	}
	...
	public function kf($order_id, $editor_id) {
		...
		PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_CANCEL, $log);
	}
	...
	public function kfAgree($order_id, $editor_id) {
		...
		PlatformOrder::where('order_id', $order_id)->update([
			'cancel_type'           => PlatformOrder::CANCEL_TYPE_KF,
			'kf_status'             => PlatformOrder::KF_STATUS_ING,
	...
	public function kfHandle($order_id, $pub_pay, $sd_pay, $handle_log, $editor_id) {
		...
		\DB::transaction(function () use ($order_id, $returnToPublisher, $toSoldier, $toPublisher, $order, $returnToSoldier, $handle_log, $editor_id) {
			...
			$data = [
				'order_status'           => PlatformOrder::ORDER_STATUS_CANCEL,
				'cancel_status'          => PlatformOrder::CANCEL_STATUS_DONE,
				'kf_status'              => PlatformOrder::KF_STATUS_DONE,
				'cancel_passed_at'       => Carbon::now(),
			...
			PlatformLog::record($order_id, $editor_id, PlatformLog::LOG_TYPE_CANCEL, '[客服处理完毕] 接单者赔偿保证金:' . $toPublisher . ', 发单者支付代练费' . $toSoldier . ', 备注 : ' . $handle_log);
	...
	public function cancelCancel($order_id, $account_id) {
		$data = [
			...
			'cancel_status'       => PlatformOrder::CANCEL_STATUS_NONE,
			'msg_sd_cancel'       => 'N',
	...
	public function quash($order_id, $editor_id = 0) {
		...
		\DB::transaction(function () use ($order, $price, $editor_id) {
			...
			// log
			PlatformLog::record($order->order_id, $editor_id, PlatformLog::LOG_TYPE_DELETE, '[撤单] 用户删除订单');

			PlatformOrder::where('order_id', $order->order_id)->update([
	...
	public function star($order_id, $star_type, $comment, $account_id) {
		$order = PlatformOrder::info($order_id);
		$type  = is_pub($order, $account_id) ? PlatformOrderStar::FROM_PUBLISHER : PlatformOrderStar::FROM_SOLDIER;
		$field = $type == PlatformOrderStar::FROM_PUBLISHER ? 'is_pub_star' : 'is_sd_star';
		// 写入 star
		$star = PlatformOrderStar::create([
		...
		// 计算各种数目
		if ($type == PlatformOrderStar::FROM_PUBLISHER) {
			// 更新打手的
			switch ($star_type) {
				case 'bad':
					PlatformOrderStar::calcSdBad($order->sd_account_id);
					break;
				case 'normal':
					PlatformOrderStar::calcSdNormal($order->sd_account_id);
					break;
				case 'good':
				default:
					PlatformOrderStar::calcSdGood($order->sd_account_id);
					break;
			}
		} else {
			// 更新发单者的
			switch ($star_type) {
				case 'bad':
					PlatformOrderStar::calcPubBad($order->account_id);
					break;
				case 'normal':
					PlatformOrderStar::calcPubNormal($order->account_id);
					break;
				case 'good':
				default:
					PlatformOrderStar::calcPubGood($order->account_id);
					break;
			}
		}
		...
		// 写入日志
		PlatformLog::record($order_id, $account_id, PlatformLog::LOG_TYPE_OVER, '[订单评价] 评价内容:' . $comment);
	}
}