...
// http://jqueryvalidation.org/jQuery.validator.format/
	...
	$.extend($.validator, {
		...
		messages : {
			...
			eqlength : $.validator.format("Please enter {0} characters."),
		...
		prototype : {
			...
			check : function (element) {
				...
				for (method in rules) {
					...
					try {
						// for metadata messages bug 2014-12-13(mark Zhao)
						if ( method == 'messages' ) continue;
			...
			metadataMessage : function (element, method) {
				if ( !$.metadata ) return {};
				var metadata = $(element).metadata({name : 'messages'});
				if ( metadata.messages ) {
					return metadata.messages[method] ? metadata.messages[method] : '';
				}
			},
			...
			defaultMessage : function (element, method) {
				return this.findDefined(
					this.metadataMessage(element, method),
		...
		methods : {
			...
			eqlength : function (value, element, param) {
				var length = $.isArray(value) ? value.length : this.getLength($.trim(value), element);
				return this.optional(element) || length == param;
			},
