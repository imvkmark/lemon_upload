define(['jquery', 'swfobject'], function($, swfobject){
	var lang = {
		_: function() {
			return '抱歉，出错啦～\n';
		},
		replace: function(file) {
			return '您之前已经添加过同名文件 ' + file + '，确定替换成新的？';
		},
		isEmpty: function(file) {
			return file + ' 是空文件！';
		},
		forbidden: function(file, accept) {
			return '不能上传 ' + file + '，只允许上传下面这些格式的文件：\n' + accept;
		},
		fileSizeLimit: function(file, size) {
			return file + ' 太大了，只能上传 ' + size + ' 以内的文件。';
		},
		queueSizeLimit: function(limit, isRemain) {
			return isRemain ?
			       '上传列队的名额（' + limit + '）所剩不多，请少选几个文件。' :
			       '一次只能同时上传 ' + limit + ' 个文件。';
		}
	};
